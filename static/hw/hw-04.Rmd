...
7) Faça uma análise descritiva da distribuição conjunta das variáveis `checking_status` e `credit_history`. De acordo com as suas análises, você acha que essas covariáveis estão correlacionadas?


8) Analise a probabilidade de $Y=1$, ou seja, classificar um cliente como mau, dado a distribuição da duração do empréstimo (variável `duration`). Para isso, siga o exemplo feito em sala para a variável idade.
