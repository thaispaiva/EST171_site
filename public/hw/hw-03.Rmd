...
date: "Entrega: **08/04/2018** via Moodle (até as 23:55)"
output: 
  html_document: 
    css: hw.css
    theme: yeti
...
- Para os exercícios a seguir, vamos usar o pacote `ggplot2` e o banco de dados de gasto de combustível `mpg`. `r emo::ji("bicycle")`
...
7) Refaça os gráficos feitos em sala para verificar a relação entre a capacidade do motor (`displ`) e o consumo do carro (`mpg`), mas com cores diferentes de acordo com o tipo de direção do carro. O que você observa na relação entre essas três variáveis?


8) Refaça o gráfico do exercício anterior, mas separe em diferentes painéis os pontos de cada tipo de direção usando o comando `facet_grid()`. Você acha que essa visualização facilita a comparação dos diferentes grupos de carros?
